...
static int get_theatre_count(FILE *file, size_t *len)
{
    ...
    theatre_t theatre;
    *len = 0;
    while (fread_theatre(&theatre, file) == EXIT_SUCCESS)
        (*len)++;
    if (!feof(file))
...
{
    fprintf(file_output, "| %-25s| %-10s|\n", "Индекс исходной таблицы", "Минимальная цена");
    for (size_t i = 0; i < keys_size; i++)
    {
        fprintf(file_output, "|------------------------|-----------------|");
        fprintf(file_output, "| %-25zu| %-10d|", table_key[i].theatre_id, table_key[i].price_low);
    }
}
...
int add_theatre(table_theatre_t *table_theatre)
{
    ...
    table_theatre->theatres[table_theatre->rows] = theatre;
    return EXIT_SUCCESS;
}
...
double time_sort_table(table_theatre_t table_theatre, size_t count, sort_t sort)
{
    ...
    for (size_t i = 0; i < count; i++)
    {
        ...
        sort(&tmp_table, tmp_table.rows, sizeof(table_theatre_t), theatre_t_comp);
...
double time_sort_keys(theatre_key_t table_keys[], size_t table_size, size_t count, sort_t sort)
{
    ...
    for (size_t i = 0; i < count; i++)
    {
        ...
        beg = microseconds_now();
        sort(&tmp_keys, table_size, sizeof(table_theatre_t), theatre_t_comp);
        end = microseconds_now();
        sum += end - beg;
    }
    return sum / count;
